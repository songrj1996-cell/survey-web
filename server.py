...
6. 主观题归纳：从 `<open_text>` 块里找该 part 内的开放题原文，先按具体题目展开，**每个具体题目必须使用 `### 题目名` 三级标题**；该题目下再按玩家态度倾向分组，分组标题必须使用 `#### 正面观点` / `#### 负面观点` / `#### 中立 / 建议` 四级标题（无相关内容的类别可省略）。严禁把「正面观点 / 负面观点 / 中立 / 建议」写成与题目平级的 `###`。在每个分组下，用表格展示各观点主题和代表性原话（列出 2–5 条原始引用，含玩家信息）。ID 展示规则固定如下：如果 `<open_text>` 前缀里只有 `MLBBID=...`，表格表头写 `MLBBID` 且单元格只放 ID 值；如果只有 `玩家ID=...`，表头写 `玩家ID` 且单元格只放 ID 值；如果两者都有，表格必须同时有 `玩家ID` 和 `MLBBID` 两列。严禁在单元格里写 `MLBB ID:xxx`、`玩家ID:xxx` 这类前缀。画像信息如需展示，必须单独放在 `玩家信息` 或 `画像信息` 列，不得混入 ID 列。
...
DEFAULT_PROMPTS: dict = {
    ...
    "writer_requirements": {
        ...
        "history": [],
        "version": 8,  # 改了默认值就 +1：未被用户编辑过的会自动升级
    },
    "planner_extra": {
        "key": "planner_extra",
        "label": "Planner 分析指令",
