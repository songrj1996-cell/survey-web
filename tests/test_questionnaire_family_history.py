...
class QuestionnaireFamilyHistoryTests(unittest.TestCase):
    ...
    def test_real_report_archive_path_keeps_only_safe_family_metadata(self):
        session = payload()
        session.update({
            "id": "history-family-save",
            "filename": "google-family.json",
            "report_md": "# Family report\n\nSafe aggregate findings.",
            "owner_key": "email:owner@example.test",
            "rows": [
                ["原因", "Google 回答来源"],
                ["PII raw answer", "en|variant-en|response-secret"],
            ],
            "google_forms_response_provenance": [{
                "respondent_email": "player@example.test",
                "original_values": ["PII raw answer"],
        ...
        self.assertEqual(
            saved["questionnaire_family_ref"]["mapping_fingerprint"],
            FINGERPRINT,
        )
        summary = family_history_summary(saved)["questionnaire_family_summary"]
        self.assertEqual(summary["languages"], ["en", "id"])
        self.assertEqual(saved["row_count"], 1)
        self.assertNotIn("rows", saved)
        self.assertNotIn("google_forms_response_provenance", saved)
        serialized = json.dumps(saved, ensure_ascii=False)
        self.assertNotIn("PII raw answer", serialized)
        self.assertNotIn("player@example.test", serialized)
