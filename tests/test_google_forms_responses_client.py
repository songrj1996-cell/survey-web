...
class GoogleFormsResponsesClientTests(unittest.IsolatedAsyncioTestCase):
    async def test_reads_all_pages_and_preserves_text_and_file_metadata(self):
        ...
        async def handler(request: httpx.Request) -> httpx.Response:
            ...
            payload = (
                {
                    "responses": [_response("response-2", value="jawaban asli")],
                    "nextPageToken": "next-1",
                }
                if token is None
                else {"responses": [_response("response-1")]}
            )
    ...
    async def test_aggregate_response_bytes_are_bounded_across_pages(self):
        first = json.dumps({
            "responses": [_response("response-1")],
            "nextPageToken": "next-1",
        }).encode()
        second = json.dumps({
            "responses": [_response("response-2")],
        }).encode()

        async def handler(request: httpx.Request) -> httpx.Response:
            content = first if request.url.params.get("pageToken") is None else second
        ...
        async with httpx.AsyncClient(
            ...
            client = GoogleFormsResponsesClient(
                ...
                lambda: {"Authorization": "Bearer synthetic-token"},
                forms_api_base=BASE,
                max_page_bytes=max(len(first), len(second)),
    ...
    def test_rejects_non_official_base_and_invalid_limits(self):
        ...
        for kwargs in (
            ...
            {"forms_api_base": BASE, "max_total_bytes": 0},
        ):
