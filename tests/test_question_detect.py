...
class QuestionDetectTests(unittest.TestCase):
    ...
    def test_profile_partial_aliases_recover_repeated_ranks_without_guessing_translation(self):
        ...
        self.assertEqual(set(result["options_original"]), set(counts))
        self.assertNotIn("unmatched_values", result)
        self.assertIn("Mythical Immortal", result["options"])
        self.assertEqual(_sanitize_choice_options(rows, [deepcopy(result)])[0], result)
    ...
    def test_profile_complete_aliases_are_preserved_without_duplicate_raw_options(self):
        ...
        self.assertEqual(result["value_aliases"], q["value_aliases"])
        self.assertNotIn("unmatched_values", result)

    def test_profile_large_vocabularies_and_long_answers_stay_pending_even_without_options(self):
        cases = [
            [f"value {i}" for i in range(100)],
            [f"value {i}" for i in range(21) for _ in range(3)],
            ["Long free-text answer about different personal situations. " + str(i)
        ...
        for values in cases:
            for declared in ({}, {"options": ["Known"], "value_aliases": {"Known": ["Known"]}}):
                with self.subTest(distinct=len(set(values)), declared=bool(declared)):
                    ...
                    self.assertTrue(all(v["suggested_handling"] == "review" for v in result["unmatched_values"]))
    ...
    def test_profile_query_requires_complete_categories_but_not_free_text_expansion(self):
        rows = [["rank"], ["Epic"], ["Legend"]]
        query = _build_column_detect_query(rows, _group_googleform_matrix(rows[0]))
        self.assertIn("固定分类须完整列出 options", query)
        self.assertIn("无法可靠翻译时保留原值", query)
        self.assertIn("不要逐条扩成标准选项", query)
