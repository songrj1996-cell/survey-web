...
from app.schemas.interview_v2_dossier import InterviewV2DossierResponse
...
class InterviewV2ParticipantDossierRerunRequest(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    from_stage: Literal["participant_dossier"]
    participant_id: str = Field(pattern=r"^participant_[0-9a-f]{32}$")
    base_dossier_version_id: str = Field(pattern=r"^dossier_[0-9a-f]{32}$")
    preserve_manual_report_edits: Literal[True] = True
    reuse_unchanged_artifacts: Literal[True] = True
    force: Literal[False] = False
...
class InterviewV2ParticipantDossierRerunResponse(InterviewV2DossierResponse):
    is_current_version: bool
    rerun: dict[str, Any]
...
InterviewV2RerunRequest = Annotated[
    InterviewV2ReportRerunRequest
    | InterviewV2AnalysisModuleRerunRequest
    | InterviewV2ParticipantDossierRerunRequest,
    Field(discriminator="from_stage"),
]
RERUN_REQUEST_ADAPTER = TypeAdapter(InterviewV2RerunRequest)
InterviewV2RerunResponse = (
    InterviewV2ReportRerunResponse
    | InterviewV2AnalysisModuleRerunResponse
    | InterviewV2ParticipantDossierRerunResponse
)
