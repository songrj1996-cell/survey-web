...
class QuestionnaireFamilyCatalogEntry(ContractModel):
    """Safe project summary without owner, Form identifiers, or diagnostics."""
...
class QuestionnaireFamilyCatalogResponse(ContractModel):
    schema_version: Literal[1] = 1
    items: list[QuestionnaireFamilyCatalogEntry] = Field(max_length=50)
...
def family_catalog_entry(
    family: QuestionnaireFamily,
) -> QuestionnaireFamilyCatalogEntry:
    summary = family_summary(family)
    return QuestionnaireFamilyCatalogEntry(
        family_id=summary.family_id,
        title=summary.title,
        status=summary.status,
        variant_count=summary.variant_count,
        languages=summary.languages,
        canonical_question_count=summary.canonical_question_count,
