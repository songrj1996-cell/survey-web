...
FamilySemanticTranslator = Callable[
    [str, str, list[FamilyVariantSnapshot], str | None],
...
class GoogleFormsFamilyNoResponsesError(GoogleFormsFamilyApiError):
    pass
...
class GoogleFormsFamilyApi:
    ...
    async def create_family(
        ...
        *,
        ...
        try:
            ...
            try:
                translated = await self.semantic_translator(
                    owner_ref,
                    title,
                    loaded,
                    llm_api_key,
            ...
            if isinstance(error, GoogleFormsQuestionnaireAuthRequiredError):
                status_code = 401
                code = "google_forms_provider_authentication_failed"
            elif isinstance(error, GoogleFormsQuestionnairePermissionError):
                status_code = 403
                code = "google_forms_provider_permission_denied"
            elif isinstance(error, GoogleFormsQuestionnaireNotFoundError):
                status_code = 404
                code = "google_forms_provider_form_not_found"
            elif isinstance(error, GoogleFormsQuestionnaireRetryableError):
                provider_status = getattr(error.__cause__, "status_code", None)
                if provider_status == 429:
                    status_code = 429
                    code = "google_forms_provider_rate_limited"
                else:
                    status_code = 503
                    code = "google_forms_provider_unavailable"
            elif isinstance(error, GoogleFormsQuestionnaireProviderError):
                status_code = 502
                code = "google_forms_provider_error"
            else:
                raise GoogleFormsFamilyInternalError() from error
        ...
        *,
        ...
        try:
            page = await asyncio.to_thread(
                self.family_storage.list_families,
                owner_ref,
                cursor=cursor,
                limit=limit,
            )
        except QuestionnaireFamilyCatalogInvalidError as error:
            raise GoogleFormsFamilyInvalidError() from error
        except QuestionnaireFamilyStorageError as error:
            raise GoogleFormsFamilyInternalError() from error
        return QuestionnaireFamilyCatalogResponse(
            items=[family_catalog_entry(family) for family in page.families],
    ...
    async def refresh_family(
        ...
        *,
        ...
        if existing is None:
            raise GoogleFormsFamilyNotFoundError()
        summary = await self.create_family(
            owner_ref,
            existing.title,
            [
                (variant.language, variant.provider_form_id)
    ...
    async def create_analysis_session(
        ...
        try:
            ...
            if len(binding.rows) <= 1:
                raise GoogleFormsFamilyNoResponsesError()
            ...
            if error.code in {
                ...
                status_code = 401
                code = "google_forms_provider_authentication_failed"
            elif error.code == GoogleFormsResponsesErrorCode.PERMISSION_DENIED:
                status_code = 403
                code = "google_forms_provider_permission_denied"
            elif error.code == GoogleFormsResponsesErrorCode.FORM_NOT_FOUND:
                status_code = 404
                code = "google_forms_provider_form_not_found"
            elif error.code == GoogleFormsResponsesErrorCode.RATE_LIMITED:
                status_code = 429
                code = "google_forms_provider_rate_limited"
            elif error.retryable or (status_code is not None and status_code >= 500):
                status_code = 503
                code = "google_forms_provider_unavailable"
            else:
                status_code = 502
