...
def _build_writer_core_query(
    ...
    return (
        ...
        f"{focus_prefix}"
        f"{mode_clause}"
        "核心结论的固定格式如下（本轮独立执行，不依赖其它轮次的格式说明）：用 `<!--CORE_START-->` 和 `<!--CORE_END-->` 两个标记"
        "**各自独占一行**包裹整段。标题必须独占一行且严格为 `## 核心结论`，不得在标题中追加样本数、分母或括号说明。"
        "标题后空一行，再用独立的斜体段落写样本附注：`*本次调研共收集 N 份有效回复。*`，不要使用 `>` 引用框；"
        "N 必须来自 <stats> 的有效样本总数，不得照抄占位符或自行推算；若未提供总数，明确说明样本总数未提供。"
        "不同题目或分支的有效回答数、分母和适用范围，也写在该附注中，保留真实口径，不得并入标题。"
        "附注后空一行，再依次写：`### 总体判断`、"
        f"{organization_clause}"
        "按需写 `### 少数但值得关注的反馈`。\n"
        f"{bug_clause}\n"
        "**信息层级与排版要求**：\n"
        "1. 样本总数之后、`### 总体判断` 下的**第一句实质判断**优先呈现与研究问题相关、证据支持最充分的重要发现，"
        "可以先写明确偏好、方案优劣、意见分歧、突出问题或尚无法判断之处。允许用关键数字支撑开头，但不要只罗列数字。"
        "证据充分的跨题洞察仍应保留；不要求每份报告都形成统一标准、决策框架或玩家共识，也不要为了组织表达凑成若干条。"
        "不同玩家分别提到不同因素，不等于他们共同遵循同一套标准。评价维度可以用于整理反馈，但不能自动写成玩家共同的"
        "决策标准或优先级；只有对应证据能够支持所声明范围内的一致程度，并交代分歧或反例，才可称为“玩家共识”。"
        "未经证据核实，不使用“玩家实际依据的是”“只有同时满足才会认可”等确定性表述。系统综合归纳出的框架属于"
        "“分析推断”，必须说明依据和适用边界，不得冒充玩家原意。直接得到支持的事实可以明确表达，不必每句话都加“可能”“似乎”。"
        "这一边界适用于整个核心结论，不能只改首句、再在后文把评价侧重写成已确定的取舍机制。不能仅凭好评或批评的主题，"
        "断言玩家如何权衡、两种标准必然对立，或把它们说成评分、投票结果的成因；只有来源直接支持这些关系时才能这样表述。"
        "认可理由不等于选择理由：来源只记录“18人认可A协调”时，应保留这一表述，不能改写为“18人因协调而选择A”。"
        "没有逐人对应或交叉统计时，不能把评价者认作选择者，也不能用“同时”“这批玩家”连接不同反馈；人数相同也不能证明是同一批玩家。"
        "有依据但未被直接证实的关系应标为“分析推断”，其余情况如实描述不同评价侧重即可。"
        "之后，`### 总体判断` 只保留有必要、重要且必须优先展示的发现、分歧或决策信息；可按真实内容写一段或多个短段，"
        ...
        + (
            ...
            "不得只给一次性案例总结，也不得用逐 Part 摘要冒充跨案例框架。交付要求不能越过证据边界："
            "证据不足时应说明暂不能形成可靠框架，或提供标为“分析推断”的待验证候选及其缺口，不得为了完成交付而声称已验证的共同标准。\n"
            if focus else ""
        )
        ...
        "不要复述、转述或重新提出业务问题或调研需求，直接写出有证据支持的发现，自然说明对象、结论和必要依据，不限定首句句式。"
...
def _build_writer_core_review_query(
    ...
    return (
        ...
        "维度、判断条件、适用边界和证据依据。交付要求不能越过证据边界：证据不足时应说明暂不能形成可靠框架，"
        "或提供标为“分析推断”的待验证候选及其缺口；这种如实说明不属于交付遗漏，不得强行修补为已验证的共同标准。\n"
        "首句应按证据与研究问题复核：检查它是否呈现与研究问题相关、证据支持最充分的重要发现，不限定首句句式。"
        "可以先写明确偏好、方案优劣、意见分歧、突出问题或尚无法判断之处。允许用关键数字支撑开头，但不要只罗列数字。"
        "不得仅因首句没有判断标准或先写有依据的评分、偏好、分歧而判定不合格，也不得仅为把框架移到首句而修补。"
        "证据充分的跨题洞察仍应保留；不要求每份报告都形成统一标准、决策框架或玩家共识，也不要为了组织表达凑成若干条。"
        "不同玩家分别提到不同因素，不等于他们共同遵循同一套标准。评价维度不能自动写成玩家共同的决策标准或优先级；"
        "只有对应证据能够支持所声明范围内的一致程度，并交代分歧或反例，才可称为“玩家共识”。未经证据核实，"
        "不使用“玩家实际依据的是”“只有同时满足才会认可”等确定性表述；应在最小必要范围内改成可直接支持的发现，"
        "或明确标为“分析推断”并说明依据和适用边界，不得冒充玩家原意。直接得到支持的事实可以明确表达，"
        "不必每句话都加“可能”“似乎”；完整保留原有统计、原因、场景、分析推断和证据边界。\n"
        "这一边界适用于整个核心结论，不能只改首句、再在后文把评价侧重写成已确定的取舍机制。不能仅凭好评或批评的主题，"
        "断言玩家如何权衡、两种标准必然对立，或把它们说成评分、投票结果的成因；只有来源直接支持这些关系时才能这样表述。"
        "认可理由不等于选择理由：来源只记录“18人认可A协调”时，应保留这一表述，不能改写为“18人因协调而选择A”。"
        "没有逐人对应或交叉统计时，不能把评价者认作选择者，也不能用“同时”“这批玩家”连接不同反馈；人数相同也不能证明是同一批玩家。"
        "逐句核对这类关系是否有来源支持；前文已经说明证据不足，不能抵消后文对同一关系的确定性断言。"
        "发现这类未经证实的关系，应局部修补为不同评价侧重，或在有依据时明确标为“分析推断”并说明边界。\n"
        ...
        f"{bug_rule}\n"
        "同时检查信息层级：`### 总体判断` 只能保留有必要、重要且必须优先展示的发现、分歧或决策信息，"
