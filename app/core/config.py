...
4. 每个 part 章节**紧接标题之后**，先写一段「本节总结」：用连贯的段落文字（不用列表）综合该 part 所有题目的客观统计结果与主观观点，要求读完这一段即可完整了解该 part 的全部发现——包括关键数据（绝对数值）、玩家态度分布、多元观点及其核心逻辑；**文字详尽，长度不限**。总结之后按业务 Topic 展开，用 `**使用现状与人群分层**`、`**核心动机与体验问题**` 这类加粗正文作为内部区隔。不得按问卷题目逐题复述；同一 Topic 下的客观题和相关开放题必须结合分析，客观统计用于说明人群背景、使用分层和判断依据，主观反馈用于完整解释原因、情境、分歧与产品含义。涉及跳转逻辑时仍须区分各分支适用人群与回答池，不得为了 Topic 汇总而混用分母或合并不同分支的反馈
5. `<stats>` 块里所有数字、百分比、表格已经算好——**严禁修改、重新计算、合并、四舍五入**。你写到报告里的所有数字必须能在 `<stats>` 里逐字找到（核心结论绝对数值也必须与 `<stats>` 一致）
6. 主观题归纳：从 `<open_text>` 中汇总该 part 内与同一业务 Topic 相关的全部开放反馈，不按具体题目或正面/负面/中立倾向机械分组。语义相同的反馈可以合并，但必须完整覆盖所有有实质信息的不同观点及其核心理由；少数玩家提出的高风险、明确案例、功能异常、流失风险或具有产品决策价值的观点也必须保留。每个观点使用固定结构：先写 `**观点：观点短标题**`，下一行写一段完整的观点说明，交代玩家表现、主要原因、使用情境、分歧和产品含义；再写 `提及情况：...`（可用「多名玩家」「少数玩家」「个别玩家」等定性量级，避免编造精确人数），最后写 `**代表性玩家反馈：**`，其下用小表格列出 1–5 条能直接支撑该观点的反馈。表头固定为 `玩家ID`、`画像信息`、`中文翻译`：所有来源和类型的身份字段统一合并到 `玩家ID` 单元格，不拆成 Discord、WhatsApp、MLBBID 等不同列；`画像信息` 只能使用 `<open_text>` 中实际存在的画像，缺失时写 `—`，不得编造；中文回答在 `中文翻译` 列原样展示，非中文回答只展示准确的中文翻译，**不得保留原始语言文本，也不得新增玩家原文列**。某观点不足 5 条时展示全部可用反馈，不得编造、重复或挪用其它观点的反馈。
