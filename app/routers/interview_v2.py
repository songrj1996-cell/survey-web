...
from app.services.interview_v2_structure_service import (
    build_structure,
    get_evidence_context,
    get_review_issues,
    get_structure,
    resolve_review_issue,
    resolve_review_issues_batch,
)
...
_STRUCTURE_JSON_MAX_BYTES = 1024 * 1024
...
async def _read_structure_json(request: Request) -> dict:
    content_type = (
        request.headers.get("content-type", "")
        .split(";", 1)[0]
        .strip()
    ...
    if content_type != "application/json" and not (
        ...
        raise ValueError("structure request must use application/json")
    content_length = request.headers.get("content-length", "").strip()
    if content_length.isdigit() and int(content_length) > _STRUCTURE_JSON_MAX_BYTES:
        raise OverflowError("structure request exceeds size limit")
    content = bytearray()
    async for chunk in request.stream():
        if len(content) + len(chunk) > _STRUCTURE_JSON_MAX_BYTES:
            raise OverflowError("structure request exceeds size limit")
        content.extend(chunk)
    value = json.loads(bytes(content).decode("utf-8"))
    if not isinstance(value, dict):
        raise ValueError("structure request must be a JSON object")
    return value
...
def _structure_request_error(
    request: Request, *, too_large: bool = False
) -> JSONResponse:
    return _error_response(
        request,
        status_code=413 if too_large else 422,
        code="STRUCTURE_REQUEST_INVALID",
        message=(
            "结构复核请求超过 1MB 上限。"
            if too_large
            else "结构复核请求格式无效。"
...
async def get_interview_import_status(import_id: str, request: Request):
    ...
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
        return await run_in_threadpool(get_interview_import, import_id, login)
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)
...
@router.post(
    ...
    response_model=InterviewV2StructureBuildResponse,
    responses={
        400: {"model": InterviewV2ErrorResponse},
        403: {"description": "沿用平台现有功能权限错误响应"},
        404: {"model": InterviewV2ErrorResponse},
...
async def build_interview_structure(import_id: str, request: Request):
    login = await _require_feature(request, "interview")
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
        raw = await _read_structure_json(request)
        payload = InterviewV2StructureBuildRequest.model_validate(raw).model_dump(
            mode="json"
        )
    except OverflowError:
        return _structure_request_error(request, too_large=True)
    except (
        json.JSONDecodeError,
        UnicodeDecodeError,
        ValidationError,
        ...
        return _structure_request_error(request)
    try:
        return await run_in_threadpool(
            build_structure, import_id, payload, login
        )
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)
...
@router.get(
    "/api/v1/interview-imports/{import_id}/structure",
    response_model=InterviewV2StructureCurrentResponse,
    responses={
        400: {"model": InterviewV2ErrorResponse},
        403: {"description": "沿用平台现有功能权限错误响应"},
        404: {"model": InterviewV2ErrorResponse},
...
async def get_interview_structure(import_id: str, request: Request):
    login = await _require_feature(request, "interview")
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
        return await run_in_threadpool(get_structure, import_id, login)
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)


@router.get(
    "/api/v1/interview-imports/{import_id}/review-issues",
    response_model=InterviewV2ReviewIssuesResponse,
    responses={
        400: {"model": InterviewV2ErrorResponse},
        403: {"description": "沿用平台现有功能权限错误响应"},
        404: {"model": InterviewV2ErrorResponse},
...
async def get_interview_review_issues(import_id: str, request: Request):
    login = await _require_feature(request, "interview")
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
        return await run_in_threadpool(get_review_issues, import_id, login)
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)
...
@router.patch(
    ...
    response_model=InterviewV2ReviewResolutionResponse,
    responses={
        400: {"model": InterviewV2ErrorResponse},
        403: {"description": "沿用平台现有功能权限错误响应"},
        404: {"model": InterviewV2ErrorResponse},
...
async def patch_interview_review_issue(issue_id: str, request: Request):
    login = await _require_feature(request, "interview")
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
        raw = await _read_structure_json(request)
        payload = InterviewV2ReviewIssuePatchRequest.model_validate(raw).model_dump(
            mode="json"
        )
    except OverflowError:
        return _structure_request_error(request, too_large=True)
    ...
    try:
        return await run_in_threadpool(
            resolve_review_issue, issue_id, payload, login
        )
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)
...
async def resolve_interview_review_issues_batch(
    import_id: str,
    request: Request,
):
    login = await _require_feature(request, "interview")
    ...
    try:
        raw = await _read_structure_json(request)
        payload = InterviewV2ReviewIssueBatchRequest.model_validate(raw).model_dump(
            mode="json"
        )
    except OverflowError:
        return _structure_request_error(request, too_large=True)
    ...
    try:
        return await run_in_threadpool(
            resolve_review_issues_batch, import_id, payload, login
        )
    except InterviewV2ImportError as exc:
        return _service_error_response(request, exc)
...
@router.get(
    "/api/v1/interview-evidence/{evidence_id}/context",
    response_model=InterviewV2EvidenceContextResponse,
    responses={
        400: {"model": InterviewV2ErrorResponse},
        403: {"description": "沿用平台现有功能权限错误响应"},
        404: {"model": InterviewV2ErrorResponse},
...
async def get_interview_evidence_context(evidence_id: str, request: Request):
    login = await _require_feature(request, "interview")
    if not INTERVIEW_V2_ENABLED:
        return _disabled_response(request)
    try:
