...
from app.core.config import (
    ...
    DEFAULT_REPORT_SOURCE_TRANSLATION_SYSTEM,
    DEFAULT_REPORT_WRITER_SYSTEM_PROMPT,
    DEFAULT_QUICK_QUESTION_SUMMARY_SYSTEM,
    DEFAULT_QUICK_BATCH_SUMMARY_SYSTEM,
    DEFAULT_QUICK_QUESTION_MERGE_SYSTEM,
    DEFAULT_INSIGHT_WRITER_REQUIREMENTS,
    DEFAULT_INSIGHT_CORE_REQUIREMENTS,
...
DEFAULT_PROMPTS: dict = {
    ...
    "quick_question_summary_system": _prompt(
        "quick_question_summary_system", "快速总结：单题归纳",
        "完整阅读一道题的原文，归纳简洁观点、粗略频次与必要风险。",
        group="报告生成与追问", group_order=20, order=34,
        kind="system", current=DEFAULT_QUICK_QUESTION_SUMMARY_SYSTEM,
    ),
    "quick_batch_summary_system": _prompt(
        "quick_batch_summary_system", "快速总结：批次提炼",
        "长题分批阅读时保留有意义的候选和严重风险，不代替整题结论。",
        group="报告生成与追问", group_order=20, order=35,
        kind="system", current=DEFAULT_QUICK_BATCH_SUMMARY_SYSTEM,
    ),
    "quick_question_merge_system": _prompt(
        "quick_question_merge_system", "快速总结：题内归并",
        "只在同一道题内归并候选，保留风险；不跨题分析或精确计数。",
        group="报告生成与追问", group_order=20, order=36,
        kind="system", current=DEFAULT_QUICK_QUESTION_MERGE_SYSTEM,
    ),
    "insight_writer_requirements": _prompt(
        "insight_writer_requirements", "观点洞察：正文写作",
        "按决策价值精简正文，同一发现只展开一次，业务含义紧随发现。",
        group="报告生成与追问", group_order=20, order=37,
        kind="instruction", current=DEFAULT_INSIGHT_WRITER_REQUIREMENTS,
    ),
    "insight_core_requirements": _prompt(
        "insight_core_requirements", "观点洞察：简短总判断",
        "用少量判断标明优先级，不重复章节解释或行动清单。",
        group="报告生成与追问", group_order=20, order=38,
        kind="instruction", current=DEFAULT_INSIGHT_CORE_REQUIREMENTS,
    ),
    "report_source_translation_system": _prompt(
        "report_source_translation_system", "报告原文：中文翻译",
        "按需完整翻译报告证据原文，保留来源对应关系，不总结或改变玩家观点。",
        group="报告生成与追问", group_order=20, order=39,
        kind="system", current=DEFAULT_REPORT_SOURCE_TRANSLATION_SYSTEM,
    ),
