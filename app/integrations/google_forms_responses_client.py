...
class GoogleFormsResponsesClient:
    ...
    async def fetch_all(self, form_id: str) -> GoogleFormResponsesCapture:
        ...
        while True:
            ...
            if remaining_bytes <= 0:
                raise _client_error(
                    ...
                    "Google Forms aggregate response payload is too large",
                )
            payload, page_bytes = await self._request_page(
                url,
                headers,
                max_bytes=min(self._max_page_bytes, remaining_bytes),
            )
        ...
        responses.sort(key=lambda item: item.response_id)
