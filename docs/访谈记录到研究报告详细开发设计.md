...
#### `EvaluationObject`：模块内被测对象、方案与 variant

真实访谈中，一个功能模块经常同时验证多个界面、概念、视频、方案组或具体 variant。它们不是新的研究问题层级，也不能只靠模块名或主问题文本隐式区分。系统在结构与证据复核完成后，必须建立独立、可确认的 `EvaluationObject`：

```text
ResearchModule
→ EvaluationObject（概念、界面或方案组）
→ EvaluationObject（可选的子级 variant）
→ MainQuestion / QuestionOccurrence
→ EvidenceEntry
```

`EvaluationObject` 至少保存：

- 所属模块、可选父对象、`concept|variant` 类型和稳定对象 ID；
- 展示名称、顺序、绑定的主问题与真实 occurrence；
- 拆分、合并时的被替代对象 ID；
- 建议、草稿、待确认和已确认状态；
- 未来可通过独立素材引用关联图片、视频或 mockup，但不复制素材正文。

系统可以按主问题和 occurrence 生成初始建议，但不得把“方案一”“第三个界面”等模糊称呼静默绑定到具体方案。用户可以在上传后的分析边界工作台中改名、排序、拆分和合并；改名与排序保持对象 ID，拆分与合并创建新对象并保留追溯关系。
...
`QuestionCoverage` 按“覆盖版本 + 玩家 + 被测对象 + 标准主问题”保存正交覆盖维度，不能把组合状态压成一个枚举。尚未建立被测对象的旧检查点不得进入玩家档案：

```json
{
  "coverage_version_id": "coverage_v05",
  "participant_id": "participant_01",
  "evaluation_object_id": "evaluation_01",
  "main_question_id": "question_team_01",
...
覆盖还必须受当前 `SourceScopeRule` 约束：

- `interview_body` 进入方案覆盖和后续正式分析；
- `participant_background` 只预留给玩家属性事实，不进入方案态度或覆盖分母；
- `excluded` 不进入档案和正式分析，但原始证据不可删除；
- 空白、只有观察或来源范围不明确时，不能推断为“未问”“不适用”或负面态度；
- 覆盖分母只有在询问状态、适用性和审核状态可靠时才能生成，否则只展示绝对证据范围。

分析标签另由 `LabelScopeRule` 控制使用范围。未配置的标签不得默认全局使用；允许范围必须显式为禁用、全局分析、指定模块或指定被测对象之一。API 固定值分别为 `disabled`、`all_analysis`、`selected_modules`、`selected_evaluation_objects`。
...
完成结构与证据复核后，同一页面进入“分析边界”检查点：

- 按模块确认具体被测对象、方案组和 variant；
- 确认哪些连续来源区段属于访谈正文、玩家背景或排除范围；
- 确认分析标签只允许用于哪些模块或被测对象；
- 预览玩家 × 被测对象 × 主问题的覆盖口径；
- 覆盖口径不可靠时只展示资料范围，不生成比例。

分析边界首次打开只生成只读建议，不写入状态。用户保存后形成不可变边界版和覆盖版；只有边界已确认、无阻断项且仍绑定当前结构/证据版本时，才能进入玩家档案。
...
5. **典型玩家逻辑**
   ...
   - 不用个案冒充总体规律。
6. **轻量产品建议**
   - 每个模块可在末尾给出少量、明确标为研究解读且绑定发现的设计启示；
   - 全文末章再去重汇总 3～5 条跨模块方向；
   - 不输出详细功能方案、优先级和排期。
7. **证据范围与研究限制**
...
- 模块章节可以呈现短小的“设计启示”，但必须明确标为研究解读并关联本模块已审计发现；
- 全文末的“轻量产品建议”章节只汇总 3～5 条去重后的跨模块方向，不复制模块原文；
- `AnalysisFinding` 可以保存建议候选，但同一方向在末章只能出现一次；
...
```http
GET  /api/v1/interview-imports/{import_id}/analysis-boundary
PUT  /api/v1/interview-imports/{import_id}/analysis-boundary
POST /api/v1/interview-imports/{import_id}/analysis-boundary:confirm
GET  /api/v1/interview-imports/{import_id}/coverage-preview
```

首次 `GET` 只生成绑定当前结构版与证据版的确定性建议，不写入项目。`PUT` 全量保存被测对象、来源范围和标签作用域，并原子地产生新的 `AnalysisBoundaryRevision` 与 `QuestionCoverageVersion`。保存请求携带当前结构、证据、边界和覆盖四个版本头；确认请求携带待确认的边界/覆盖双头与双摘要，服务端再校验其冻结的结构/证据来源。任一上游头变化都返回冲突并要求刷新，不能静默覆盖用户编辑。

来源范围只影响下游资格和覆盖，不删除或改写原始 `EvidenceEntry`。覆盖预览中的比例和分母完全来自服务端已确认口径，前端不得自行用可见行数计算。
...
### 15.5.1 批次 3B：分析边界与方案级覆盖

#### 目标

在不提高记录员输入要求的前提下，把已确认结构转换成可供玩家档案使用的分析边界，避免同一模块内多个界面或方案被混写。

#### 工作项

- 在模块与主问题之间建立可确认的 `EvaluationObject` 和可选 variant；
- 支持对象改名、排序、拆分、合并及替代关系追溯；
- 将来源连续区段确认成访谈正文、玩家背景或排除区段；
- 让来源范围真实过滤方案覆盖，但不修改或删除原始证据；
- 显式保存分析标签可使用的模块或被测对象范围；
- 生成“玩家 × 被测对象 × 主问题”的正交覆盖预览；
- 使用绑定当前结构版和证据版的不可变边界版、覆盖版及乐观并发控制；
- 结构或证据变化后将旧分析边界标记为过期。

#### 输入合同边界

- 不增加 Excel 必填列、方案 ID、覆盖状态或标签字段；
- 不升级批次 1 的文件合同；
- 模糊方案名称进入上传后的人工确认，不要求记录员重新整理或上传；
- 样式、颜色和位置只能生成来源或分群候选，不能自动成为已确认研究事实。

#### 验收

- 同一模块可以独立保存多个方案和 variant，证据不会跨方案静默串用；
- 背景区和排除区不进入方案覆盖，原始证据仍可追溯；
- 未配置的分析标签不会扩散到所有模块；
- 空白、观察-only、未问、不适用和无记录不会相互替代；
- 多记录员记录同一玩家时来源可为多条，玩家人数仍只计一次；
- 覆盖分母不可靠时不输出比例；
- 未确认或过期的分析边界不能进入正式玩家档案。
